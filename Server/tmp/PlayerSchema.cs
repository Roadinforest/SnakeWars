...
public partial class PlayerSchema : Schema {
	...
	[Type(5, "uint16")]
	public ushort lives = default(ushort);
	...
	protected event PropertyChangeHandler<ushort> __livesChange;
	public Action OnLivesChange(PropertyChangeHandler<ushort> __handler, bool __immediate = true) {
		if (__callbacks == null) { __callbacks = new SchemaCallbacks(); }
		__callbacks.AddPropertyCallback(nameof(this.lives));
		__livesChange += __handler;
		if (__immediate && this.lives != default(ushort)) { __handler(this.lives, default(ushort)); }
		return () => {
			__callbacks.RemovePropertyCallback(nameof(lives));
			__livesChange -= __handler;
		};
	}
	...
	protected override void TriggerFieldChange(DataChange change) {
		switch (change.Field) {
			...
			case nameof(lives): __livesChange?.Invoke((ushort) change.Value, (ushort) change.PreviousValue); break;
			default: break;
		}
	}
}
